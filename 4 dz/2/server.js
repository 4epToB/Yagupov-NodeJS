...
let mimeTypes = {//Далее создадим объект с mimе-типами:
    '.html': 'text/html',
...
http.createServer((request, response) => {// вызов метода создания http сервера
    ...
    if (extname === '.mp4') {
        ...
        let responseHeaders = {};//создаем пустой объект для будущего хранения заголовков ответа сервера
        let stat = fs.statSync(pathname);//Функция statSync возвращает описание файла.
        ...
        fs.read(fd, buf, 0, buf.length, start, (err, bytes) => {
            if(err){
                ...
                response.end();
            } else { //Отдаем часть файла и соответствующий статус 206
